...
task_s = {}
task_s['s']  = [[180],' -DSMALL_DATASET'] 
task_s['sm'] = [[340],' -DSM_DATASET'] 
task_s['m']  = [[500],' -DMEDIUM_DATASET'] 
task_s['ml'] = [[1650],' -DML_DATASET'] 
task_s['l']  = [[2800],' -DLARGE_DATASET'] 
task_s['xl'] = [[5600],' -DEXTRALARGE_DATASET'] 

input_sizes= {}
input_sizes[tuple([180])]  = [' -DSMALL_DATASET'] 
input_sizes[tuple([340])] = [' -DSM_DATASET'] 
input_sizes[tuple([500])] = [' -DMEDIUM_DATASET'] 
input_sizes[tuple([1650])] = [' -DML_DATASET'] 
input_sizes[tuple([2800])] = [' -DLARGE_DATASET'] 
input_sizes[tuple([5600])] = [' -DEXTRALARGE_DATASET']
################################### 
...
def create_gptune(Kernel_name, nodes, cores, n_param, param_start, param_name, i_min, i_max, task_name):
    ...
    ## tuning parameter
    Tuning_params = []
    p1 = Categoricalnorm(["#pragma clang loop(j2) pack array(path) allocate(malloc)", " "], transform="onehot", name="p1") 
...
if __name__ == "__main__":
    ...
    for i, ss in zip(range(1,len(giventask),1),source_task):
        ...
        meta_dict = {
            ...
            "input_space":i_space,
            "parameter_space": [
        {"name": "p1","transformer": "onehot","type": "categorical","categories": ["#pragma clang loop(j2) pack array(path) allocate(malloc)", " "]},
